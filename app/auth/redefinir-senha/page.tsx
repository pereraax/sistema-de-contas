...
function RedefinirSenhaContent() {
  ...
  // Processar hash, token_hash ou code ao montar
  useEffect(() => {
    const processarAuth = async () => {
      const supabase = createClient()

      // Verificar code na query (fluxo PKCE)
      ...
      // Verificar token_hash na query (fluxo Supabase)
      ...
      // Verificar hash (#access_token) - fluxo implícito
      ...
      if (hash && hash.includes('access_token')) {
        ...
        if (accessToken) {
          ...
          if (error) {
            setErro('Erro ao processar link. Solicite um novo.')
            setStatus('error')
            return
          }
          // Limpar hash da URL
