...
export async function DELETE() {
  try {
    ...
    // 1) Desvincular sessão WhatsApp (para este usuário) – na próxima mensagem o sistema pede email + chave de novo
    ...
    if (sessionError) {
      if (!sessionError.message?.includes('does not exist') && sessionError.code !== '42P01') {
        console.warn('[whatsapp-key DELETE] Aviso ao limpar whatsapp_sessions:', sessionError.message)
      }
    }

    // 2) Remover chave do perfil (obriga a gerar nova chave para reconectar)
    const { error: updateError } = await admin
      .from('profiles')
    ...
    return NextResponse.json({
      success: true,
      message: 'Chave e sessão WhatsApp desvinculadas. Gere uma nova chave e envie pelo WhatsApp (email + chave) para reconectar.',
    })
  } catch (err: unknown) {
