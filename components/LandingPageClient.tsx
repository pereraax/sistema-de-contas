...
export default function LandingPageClient() {
  ...
  useEffect(() => {
    ...
    if (hasCode || hasOAuthHash) {
      if (!params.has('next')) params.set('next', isApp ? '/onboarding' : '/home')
      if (isApp && !params.has('platform')) params.set('platform', 'app')
  ...
  // No app (Capacitor): nunca ficar na landing.
  // Se logado e ainda não completou onboarding → /onboarding; senão /home.
  useEffect(() => {
    if (!isApp || !isAuthenticated) return
    let cancelled = false
    const go = async () => {
      try {
        const supabase = createClient()
        const { data: { user } } = await supabase.auth.getUser()
        if (cancelled) return
        const completed = (user?.user_metadata as Record<string, unknown> | undefined)?.app_onboarding_completed_at
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 max-w-7xl mx-auto items-start">
            ...
            <AnimateOnScroll delay={150} direction="up">
              ...
              <div className="mb-3 sm:mb-4">
                <div className="flex items-baseline gap-2">
                          <span className={`text-xl sm:text-2xl md:text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-[#0D1B2A]'}`}>R$ 9,90</span>
