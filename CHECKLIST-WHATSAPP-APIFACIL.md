...
## 4. Se a assistente não responde NENHUMA mensagem

- **Primeiro:** Nos logs do Railway, procure por `WEBHOOK CHAMADO | APIFACIL configurado:`. Se estiver **false**, a assistente nunca envia resposta — defina **APIFACIL_INSTANCE_ID** e **APIFACIL_TOKEN** no painel do Railway (Variáveis de ambiente) e faça **Redeploy**.
- Se aparecer **"Resposta que seria enviada para..."** nos logs, o processamento está OK e só falta a API Fácil configurada (mesmo passo acima).

## 5. Se ainda não responder (outros casos)

- Confira os **logs do Railway** (Deploy Logs / HTTP Logs): procure por `[Apifacil Webhook]` e `[WhatsApp PLEN]`. Se aparecer "Payload não reconhecido", o formato que a Apifacil envia pode ter mudado (veja "Keys:" no log).
...
- Se o webhook não for chamado (nada nos logs ao enviar mensagem), a URL no Apifacil pode estar errada ou inativa — confira item 1.
